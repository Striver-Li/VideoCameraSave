...
bool VideoDecode::open(const QString &url)
{
     ...
     * Linux：可使用【ffmpeg -list_formats all -i /dev/video0】或【ffplay -f video4linux2 -list_formats all /dev/video0】
     *        命令查看摄像头支持的支持的像素格式、编解码器和帧大小
     */
    // 设置解码器
    // AV_OPT_SEARCH_CHILDREN: 首先搜索给定对象的可能子对象
    av_dict_set(&dict, "input_format", "mjpeg", AV_OPT_SEARCH_CHILDREN);
    // 设置帧率
    av_dict_set(&dict, "framerate", "25", 0);
    // 设置视频分辨率（如果该分辨率摄像头不支持则会报错）
...
//    m_codecContext->thread_count = 8;               // 使用8线程解码
     ...
     *         但是少部分视频图像在使用sws_scale()拷贝时会超出数组长度
     */
...
void VideoDecode::free()
{
    ...
    // 关闭并释放m_formatContext，并将指针置为null
    if(m_formatContext) {
