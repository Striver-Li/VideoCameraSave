...
bool VideoSave::open(AVStream *inStream, const QString &fileName)
{
     ...
     */
    // 获取编码器名称
    QString strName = avcodec_find_encoder(inStream->codecpar->codec_id)->name;
    // 这里使用和解码一样的编码器，防止保存的图像颜色出问题
    int ret = avformat_alloc_output_context2(&m_formatContext, nullptr, strName.toStdString().data(), fileName.toStdString().data());
#endif
    if(ret < 0) {
        close();
        showError(ret);
    ...
    /**
     * @brief 创建并初始化AVIOContext以访问url所指示的资源
     * @param s 用于返回指向创建的AVIOContext的指针，失败返回NULL。
     * @param url 资源访问
     * @param flags 控制由url指示的资源如何打开的标志
     */
...
#else
    // 像素格式，也可以使用codec->pix_fmts[0]或AV_PIX_FMT_YUVJ422P(【注意】摄像头解码的图像格式为yuvj422p，如果这里不一样可能保存会出问题，或者后面进行格式转换)
    m_codecContext->pix_fmt = AVPixelFormat(inStream->codecpar->format);
#endif
...
void VideoSave::write(AVFrame *frame)
{
    ...
    while (true) {
        ...
        // 将数据包中的有效时间字段（时间戳/持续时间）从一个时基转换为输出流的时间
